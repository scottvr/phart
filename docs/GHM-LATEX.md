...
I'm _almost there_.  Long story a little less long that it could be, _**GitHub-Flavored Markdown**_ **(_GFM_)** - ("GHFM"? GHFMD"? Something of the sort.) while it does allow embedding of, wouldn't-cha-know, _mermaid syntax_, which then converts your short little text-symbolic description of the flow to an SVG in your markdown using the **mermaidjsAPI**, but it's not what I wanted. I wanted the text, as it came out of **phart**. 
