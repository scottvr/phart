...
def is_node_left_most(node):
    if node.is_left and node.has_parent:
        return is_node_left_most(node.parent)
    elif (not node.is_left) and node.has_parent:
        return False
    else:
        return True
...
def generate_tree(depth=5):
    ...
    while current_depth < depth:
        ...
        for node in nodes:
            if (
                isinstance(node, Zero)
                or isinstance(node, Frac)
                or isinstance(node, Exists)
            ):
                continue
...
class CollatzPrinter:
    ...
    def render(self):
        ...
        options = LayoutOptions(
            node_spacing=2, binary_tree_layout=True, layer_spacing=3, use_ascii=True
        )
